...
T3 Code is a minimal GUI for coding agents. A Node WebSocket server wraps provider CLIs and agents (Codex, Claude Code, Cursor, Cline, Grok, OpenCode, Antigravity) and serves web, desktop, and mobile clients.
...
- **Providers.** Codex, Claude, Cursor, Cline, Grok, OpenCode, and Antigravity each have an adapter. Provider-shaped features need a decision per adapter, even if the decision is "not supported here".
