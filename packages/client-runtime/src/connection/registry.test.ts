...
const makeHarness = Effect.fn("TestEnvironmentRegistry.makeHarness")(function* (
  ...
  const cloudSandboxGateway = ClientCapabilities.CloudSandboxEnvironmentGateway.of({
    provision: () => Effect.die(new Error("Cloud sandbox provisioning is not used.")),
    prepare: () => Effect.die(new Error("Cloud sandbox preparation is not used.")),
  ...
  const layer = EnvironmentRegistry.layer.pipe(
    Layer.provide(
      Layer.mergeAll(
        ...
        Layer.succeed(ClientCapabilities.CloudSandboxEnvironmentGateway, cloudSandboxGateway),
